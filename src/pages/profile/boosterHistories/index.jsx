...
const PriceChip = styled(Chip)(({ theme }) => ({
  ...
  color: "white",
  boxShadow: "0 2px 6px rgba(254, 107, 139, 0.3)",
...
const StatusChip = styled(Chip)(({ status, theme }) => ({
  ...
  background:
    status === "ACTIVE"
      ...
      : status === "COMPLETED"
      ? "linear-gradient(135deg, #ff4d4f 0%, #ff3333 100%)"
      : "linear-gradient(135deg, #8e8e8e 0%, #bdbdbd 100%)",
  color: "white",
  boxShadow:
    status === "ACTIVE"
      ? "0 2px 6px rgba(56, 239, 125, 0.3)"
      : status === "COMPLETED"
      ? "0 2px 6px rgba(255, 0, 0, 0.3)"
      : "0 2px 6px rgba(189, 189, 189, 0.3)",
  padding: theme.spacing(0, 0.5),
  height: 20,
  fontSize: 10,
}));
...
export default function BoosterHistories() {
  const [loading, setLoading] = useState(true);
  const [boosterHistories, setBoosterHistories] = useState([]);
  ...
  const fetchAllHistories = async (page = 1) => {
    ...
    try {
      ...
      if (response && response.data?.responseCode === 200) {
        ...
        setBoosterHistories(docs || []);
        setCurrentPage(currentPg);
        setTotalPages(pages);
        setLimit(resLimit);
        setBoosterCount(total);
      }
    } catch (error) {
      console.error("Error fetching histories: ");
    } finally {
    ...
    <PageContainer>
          ...
          <Box sx={{ position: "relative", zIndex: 1 }}>
            ...
            <CountBadge>
              ...
              <Typography variant="caption">
                Total Boosters: {loading ? "..." : boosterCount}
              </Typography>
            </CountBadge>
          </Box>
          <IconButton
      ...
      <Box sx={{ p: 2, pt: 2, position: "relative", zIndex: 1, flex: 1 }}>
        {loading ? (
          ...
          <>
            {boosterHistories.map((booster, index) => (
              <StyledCard
                key={booster._id}
                sx={{
                ...
                <Box sx={{ p: 2 }}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {formatMinutesAsTime(
                          booster.Booster_Id.Timer_InMinutes
                        )}
                      </Typography>
                    </Box>
                  </Box>
